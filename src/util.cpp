...
const char * const BITCOIN_CONF_FILENAME = "rimecoin.conf";
const char * const BITCOIN_PID_FILENAME = "rimecoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rimecoin" is a composite category enabling all Rimecoin-related debug output
            if(ptrCategory->count(string("rimecoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RimecoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RimecoinCore
    // Mac: ~/Library/Application Support/RimecoinCore
    // Unix: ~/.rimecoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RimecoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RimecoinCore";
#else
    // Unix
    return pathRet / ".rimecoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rimecoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rimecoin.conf
